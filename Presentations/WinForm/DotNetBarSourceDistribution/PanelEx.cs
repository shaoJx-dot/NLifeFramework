...
namespace DevComponents.DotNetBar
{
	...
	/// </summary>
    [ToolboxItem(true), Designer("DevComponents.DotNetBar.Design.PanelExDesigner, DevComponents.DotNetBar.Design, Version=11.4.0.0, Culture=neutral,  PublicKeyToken=90f470f34c89ccaf"), DefaultEvent("Click")]
    public class PanelEx : System.Windows.Forms.Panel, IButtonControl, Controls.INonClientControl
	{
		...
		private void SetRegion(bool bResize)
		{
			if(bResize || m_CurrentCornerType!=m_Style.CornerType || m_CornerDiameter!=m_Style.CornerDiameter)
			{
				...
				{
                    Region newRegion=m_Style.GetRegion(this.ClientRectangle);
        ...
        public bool TextMarkupEnabled
        {
            ...
            set
            {
                if (_TextMarkupEnabled != value)
                {
                    ...
                    if (_TextMarkupEnabled) ResizeMarkup();
                    this.Invalidate();
                }
            }
        }
        ...
        public Size GetAutoSize(int preferedWidth)
        {
            ...
            else if (this.Text.Length > 0)
            {
                ...
                {
                    if(preferedWidth<=0)
                        size = TextDrawing.MeasureString(g, this.Text, font, 0, (this.Style.UseMnemonic ? eTextFormat.Default : eTextFormat.NoPrefix));
                    else
                        size = TextDrawing.MeasureString(g, this.Text, font, preferedWidth, (this.Style.UseMnemonic ? eTextFormat.Default : eTextFormat.NoPrefix));
                }
                size.Width += 2;
                size.Height += 2;
            }
