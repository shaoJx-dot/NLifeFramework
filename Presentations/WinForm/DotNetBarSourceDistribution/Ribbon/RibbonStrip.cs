    ...
    public class RibbonStrip : ItemControl
    {
        ...
        internal override bool IsGlassEnabled
        {
            get
            {
                ...
                if (form is OfficeForm) return false;
                if (form is RibbonForm)
                    return ((RibbonForm)form).IsGlassEnabled;
        ...
        /// </summary>
        [Browsable(true), DefaultValue(""), Editor("DevComponents.DotNetBar.Design.TextMarkupUIEditor, DevComponents.DotNetBar.Design, Version=11.4.0.0, Culture=neutral,  PublicKeyToken=90f470f34c89ccaf", typeof(System.Drawing.Design.UITypeEditor)), EditorBrowsable(EditorBrowsableState.Always), Category("Appearance"), Description("Indicates text displayed on Ribbon Title instead of the Form.Text property.")]
        ...
        /// </summary>
        [Editor("DevComponents.DotNetBar.Design.RibbonTabItemGroupCollectionEditor, DevComponents.DotNetBar.Design, Version=11.4.0.0, Culture=neutral,  PublicKeyToken=90f470f34c89ccaf", typeof(System.Drawing.Design.UITypeEditor)), DesignerSerializationVisibility(DesignerSerializationVisibility.Content), Category("Tab Groups"), Browsable(true), DevCoBrowsable(true)]
        ...
        protected override void WndProc(ref Message m)
        {
            if (m.Msg == (int)WinApi.WindowsMessages.WM_NCHITTEST)
            {
                ...
                if (m_CaptionVisible && (effectiveStyle == eDotNetBarStyle.Office2010 || StyleManager.IsMetro(effectiveStyle)) && (p.X < 28 && this.RightToLeft == RightToLeft.No || p.X > this.Width - 28 && this.RightToLeft == RightToLeft.Yes) && p.Y < 28)
                {
                    m.Result = new IntPtr((int)WinApi.WindowHitTestRegions.TransparentOrCovered);
                    return;
                }
                else if (m_CaptionVisible && effectiveStyle == eDotNetBarStyle.Windows7 && (p.X < 28 && this.RightToLeft == RightToLeft.No || p.X > this.Width - 28 && this.RightToLeft == RightToLeft.Yes) && p.Y < 28)
                {
                    m.Result = new IntPtr((int)WinApi.WindowHitTestRegions.TransparentOrCovered);
                    return;
                }
        ...
        {
            if (this.IsGlassEnabled && (this.EffectiveStyle == eDotNetBarStyle.Office2010 || StyleManager.IsMetro(this.EffectiveStyle)))
            {
        ...
        protected override void PaintControlBackground(ItemPaintArgs pa)
        {
            ...
            RibbonForm ribbonForm = this.FindForm() as RibbonForm;

            if (isGlassEnabled)
            {
                if (ribbonForm != null)
                {
                    pa.Graphics.SetClip(new Rectangle(0, 0, this.Width, ribbonForm.GlassHeight - ((effectiveStyle == eDotNetBarStyle.Office2010 || StyleManager.IsMetro(effectiveStyle)) && pa.GlassEnabled ? Office2010GlassExcludeTopPart : 1)),
                        System.Drawing.Drawing2D.CombineMode.Exclude);
                    resetClip = true;
                }
            }

            if (isGlassEnabled && (effectiveStyle == eDotNetBarStyle.Office2010 || StyleManager.IsMetro(effectiveStyle)))
            {
            ...
            RibbonControl ribbon = this.Parent as RibbonControl;
            if (renderer != null && ribbon != null)
            {
                ...
                if (ribbon.BackgroundImage != null && !this.IsBackstageOpen)
                {
                    Rectangle br = GetBackgroundImageRect(ribbonForm);
                    BarFunctions.PaintBackgroundImage(pa.Graphics, br, ribbon.BackgroundImage,
                        ribbon.BackgroundImagePosition, 255);
                }
        ...
        private Rectangle GetBackgroundImageRect(RibbonForm ribbonForm)
        {
            Rectangle br = this.ClientRectangle;
            if (ribbonForm != null)
                br.Inflate(ribbonForm.BorderSize - 1, 0);
            return br;
        }

        private bool _TabGroupPaintSuspended = false;
        /// <summary>
        /// Gets or set whether TabGroup painting is suspended.
        /// </summary>
        internal bool TabGroupPaintSuspended
        {
            get { return _TabGroupPaintSuspended; }
            set
            {
                if (_TabGroupPaintSuspended != value)
                {
                    _TabGroupPaintSuspended = value;
                    this.Invalidate();
                    Form form = this.FindForm();
        ...
        internal void PaintTabGroups(ItemPaintArgs pa)
        {
            if (m_Groups.Count == 0 || _TabGroupPaintSuspended)
                return;
                    ...
                    if (tab.Group != currentGroup)
                    {
                        if (tab.Visible)
                        {
                            ...
                            if (currentGroup != null)
                            {
                                ...
                                groupRect = new Rectangle(tab.DisplayRectangle.X, y, tab.DisplayRectangle.Width, ((effectiveStyle == eDotNetBarStyle.Office2010 || StyleManager.IsMetro(effectiveStyle)) ? tab.DisplayRectangle.Y : tab.DisplayRectangle.Y - 4));
                                groupBounds = new Rectangle(tab.DisplayRectangle.X, y, tab.DisplayRectangle.Width, tab.DisplayRectangle.Bottom - (StyleManager.IsMetro(effectiveStyle) ? 1 : m_TabGroupHeight));
                            }
                        }
                    }
        ...
        private int GetCaptionHeight()
        {
            if (m_CaptionHeight == 0)
            {
                ...
                if (this.IsGlassEnabled && EffectiveStyle != eDotNetBarStyle.Office2010 && EffectiveStyle != eDotNetBarStyle.Windows7 && !StyleManager.IsMetro(EffectiveStyle))
                    captionHeight += 0;
                    ...
                    captionHeight -= 2;
                else if (!this.IsGlassEnabled && StyleManager.IsMetro(EffectiveStyle))
                    captionHeight -= 2;
        ...
        {
            return GetCaptionHeight() + m_TabGroupHeight + ((this.EffectiveStyle == eDotNetBarStyle.Office2010 || this.EffectiveStyle == eDotNetBarStyle.Windows7 || StyleManager.IsMetro(this.EffectiveStyle)) ? 0 : 0);
        }
        ...
        private bool m_AutoKeyboardExpand = true;
        /// <summary>
        /// Gets or sets whether control is collapsed when RibbonTabItem is double clicked and expanded when RibbonTabItem is clicked.
        /// </summary>
        internal bool AutoKeyboardExpand
        {
            get { return m_AutoKeyboardExpand; }
            set { m_AutoKeyboardExpand = value; }
        }
        ...
        protected override bool OnKeyDown(IntPtr hWnd, IntPtr wParam, IntPtr lParam)
        {
            ...
            else if (wParamInt == 112 && Control.ModifierKeys == Keys.Control)
            {
        ...
        private bool _IgnoreF10Key = false;
        /// <summary>
        ...
        {
            int wParamInt = WinApi.ToInt(wParam);
            if (wParamInt == 18 || wParamInt == 121 && !_IgnoreF10Key)
            {
                bool callBase = true;
                if (m_AltFocus)
                {
        ...
        internal bool IsBackstageOpen
        {
            get
            {
                ApplicationButton appButton = GetApplicationButton() as ApplicationButton;
                if (appButton != null && appButton.BackstageTab != null && appButton.Expanded && appButton.BackstageTabEnabled)
                    return true;
                return false;
            }
        }
            ...
            if (m_CaptionVisible)
            {
                ...
                if (e.Button == MouseButtons.Right && m_MouseDownOnCaption)
                {
                    ...
                    if (form != null)
                    {
                        ...
                        BarUtilities.InvokeDelayed(new MethodInvoker(delegate { m_MouseDownOnCaption = false; }), 500);
                        return;
                    }
                }
        ...
        private RibbonTabItemGroup HitTestTabItemGroup(Point p)
        {
            if (m_CaptionVisible && m_TabGroupsVisible && !m_StripContainer.RibbonStripContainer.DisplayRectangle.Contains(p.X, p.Y))
            {
                foreach (RibbonTabItemGroup group in this.TabGroups)
                {
                    if (!group.Visible) continue;
        ...
        internal BaseItem GetApplicationButton()
        {
            ...
            if ((this.EffectiveStyle == eDotNetBarStyle.Office2010 || StyleManager.IsMetro(this.EffectiveStyle)) && this.Items.Count > 0)
            {
                foreach (BaseItem item in this.Items)
                {
                    if (item is ApplicationButton) return item;
                }
                //return this.Items[0];
            }

            if (cont.SubItems.Count > 0 && cont.SubItems[0] is ApplicationButton)
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            if (m_CaptionVisible)
            {
                ...
                {
                    Form form = this.FindForm();
                    if (form != null && (form.WindowState == FormWindowState.Normal || BarFunctions.IsWindows7))
                    {
        ...
        protected bool HitTestCaption(Point p)
        {
            ...
            if (BarFunctions.IsOffice2010Style(this.EffectiveStyle) && this.ClientRectangle.Contains(p))
            {
                if (HitTest(p.X, p.Y) == null && HitTestTabItemGroup(p) == null)
                    return true;
            }
            return false;
        }
