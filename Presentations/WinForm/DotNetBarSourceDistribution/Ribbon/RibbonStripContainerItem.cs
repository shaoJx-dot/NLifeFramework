            ...
            if (m_RibbonStrip.CaptionVisible)
            {
                ...
                if (m_RibbonStrip.CaptionHeight == 0 && m_SystemCaptionItem.TopInternal < (frameBorderSize.Height - 1))
                {
                    if (System.Environment.OSVersion.Version.Major >= 6 && !isGlassEnabled)
                    {
                        ...
                        {
                            m_SystemCaptionItem.TopInternal = frameBorderSize.Height - 4;
                            if (_WindowIcon != null)
                            {
                                //if(BarFunctions.IsWindows8)
                                //    _WindowIcon.TopInternal = frameBorderSize.Height - 1;
                                //else
                                    _WindowIcon.TopInternal = Math.Max(0, frameBorderSize.Height - 5);
                            }
                        }
                    }
                    else
                    {
                        ...
                        if (isGlassEnabled)
                        {
                            if (_WindowIcon != null) _WindowIcon.TopInternal = frameBorderSize.Height - 2;
                ...
                if (System.Environment.OSVersion.Version.Major >= 6)
                {
                    ...
                    foreach (BaseItem item in m_CaptionContainer.SubItems)
                    {
                        if (!(item is ApplicationButton) && item != m_SystemCaptionItem)
