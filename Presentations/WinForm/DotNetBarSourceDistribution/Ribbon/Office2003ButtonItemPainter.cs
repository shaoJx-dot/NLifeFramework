...
namespace DevComponents.DotNetBar
{
	...
	internal class Office2003ButtonItemPainter:ButtonItemPainter
    {
		...
		public override void PaintButtonImage(ButtonItem button, ItemPaintArgs pa, CompositeImage image, Rectangle imagebounds)
		{
            ...
            {
                Font font = button.SymbolSize > 0 ? Symbols.GetFontAwesome(button.SymbolSize) : Symbols.FontAwesome;
                if (isOnMenu)
                    TextDrawing.DrawStringLegacy(pa.Graphics, button.Symbol, font, GetTextColor(button, pa), new Rectangle(imagebounds.X + imagebounds.Width / 2, imagebounds.Y, 0, 0), eTextFormat.Default | eTextFormat.HorizontalCenter);
                else
                    TextDrawing.DrawStringLegacy(pa.Graphics, button.Symbol, font, GetTextColor(button, pa), new Rectangle(imagebounds.X, imagebounds.Y+1  , 0, 0), eTextFormat.Default);
                return;
            }
		...
		public override void PaintButtonCheck(ButtonItem button, ItemPaintArgs pa, CompositeImage image, Rectangle r)
		{
			...
			if((image==null && string.IsNullOrEmpty(button.Symbol) || button.ButtonStyle==eButtonStyle.TextOnlyAlways) && isOnMenu)
			{
				// Draw checker...
		...
		public override eTextFormat GetStringFormat(ButtonItem button, ItemPaintArgs pa, CompositeImage image)
		{
			...
			if(!isOnMenu)
			{
				if(pa.ContainerControl is RibbonStrip && (image==null && string.IsNullOrEmpty(button.Symbol) || button.ImagePosition==eImagePosition.Top || button.ImagePosition==eImagePosition.Bottom)
                ...
                else if (pa.ContainerControl is ButtonX)
                {
                    ...
                    else if (buttonX.TextAlignment == eButtonTextAlignment.Right && (image == null && string.IsNullOrEmpty(button.Symbol) || button.ImagePosition == eImagePosition.Top || button.ImagePosition == eImagePosition.Bottom))
                        stringFormat |= eTextFormat.Right;
                }
                else if (pa.IsOnMenuBar || (pa.ContainerControl is Bar || pa.ContainerControl is ButtonX || button.Orientation == eOrientation.Vertical) && image == null && string.IsNullOrEmpty(button.Symbol) || button.ImagePosition == eImagePosition.Top || button.ImagePosition == eImagePosition.Bottom)
                    stringFormat |= eTextFormat.HorizontalCenter;
        ...
        protected virtual bool IsTextCentered(ButtonItem button, ItemPaintArgs pa, CompositeImage image)
        {
            ...
            if (!isOnMenu)
            {
                ...
                if (cc is RibbonStrip && (image == null && string.IsNullOrEmpty(button.Symbol) || button.ImagePosition == eImagePosition.Top || button.ImagePosition == eImagePosition.Bottom) || button.Name.StartsWith("sysgallery") || button._FixedSizeCenterText)
                    return true;
                ...
                else if (pa.IsOnMenuBar || cc is Bar && image == null && string.IsNullOrEmpty(button.Symbol) || (button.ImagePosition == eImagePosition.Top || button.ImagePosition == eImagePosition.Bottom))
                    return true;
            }
