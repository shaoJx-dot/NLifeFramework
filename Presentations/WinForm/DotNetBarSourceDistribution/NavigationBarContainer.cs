...
namespace DevComponents.DotNetBar
{
    ...
    public class NavigationBarContainer : ImageItem, IDesignTimeProvider
	{
		...
		public override void RecalcSize()
		{
			...
			foreach(BaseItem item in this.SubItems)
			{
				if(item.Visible)
				{
					if(y+item.HeightInternal+(lastVisible==item?0:itemSpacing)<=m_Rect.Bottom-(m_SummaryLineVisible?iMaxHeight:0)) //((!bFirstInLine && lastVisible==item)?0:iMaxHeight))
					{
                        ...
                        if (lastVisible == item) heightActual -= itemSpacing;
                        item.DesignMarkerOrientation = eDesignMarkerOrientation.Vertical;
					}
					else
					{
		...
		private void PaintLineBackground(ItemPaintArgs pa,Rectangle r, eDotNetBarStyle style)
		{
            ...
            if (style != eDotNetBarStyle.Office2010 && !StyleManager.IsMetro(style))
            {
		...
		public override void Paint(ItemPaintArgs pa)
		{
			...
			eDotNetBarStyle effectiveStyle = this.EffectiveStyle;
            if ((effectiveStyle == eDotNetBarStyle.Office2010 || StyleManager.IsMetro(effectiveStyle)) && pa.Renderer!=null)
            {
                Region oldClip = pa.Graphics.Clip;
                pa.Graphics.ResetClip();
                pa.Renderer.DrawNavPaneButtonBackground(new NavPaneRenderEventArgs(pa.Graphics, pa.ContainerControl.ClientRectangle));
                pa.Graphics.Clip = oldClip;
                if (oldClip != null) oldClip.Dispose();
            }
            if(m_ConfigureItem!=null && m_SummaryLineVisible)
			{
				Rectangle rBottomLine=new Rectangle(m_Rect.X,m_ConfigureItem.TopInternal,m_Rect.Width,m_ConfigureItem.HeightInternal);
                PaintLineBackground(pa, rBottomLine, effectiveStyle);
			}
            ...
            bool paintItemBackground = (pa.Colors.ItemBackground.IsEmpty && pa.Colors.ItemBackground2.IsEmpty) && effectiveStyle != eDotNetBarStyle.Office2010;
			for(int i=0;i<this.SubItems.Count;i++)
			{
				...
				if(item.Visible && item.Displayed)
				{
                    ...
                    {
                        if (BarFunctions.IsOffice2007Style(effectiveStyle) && pa.Renderer != null)
					...
					item.Paint(pa);
                    if (i > 0 && effectiveStyle != eDotNetBarStyle.Office2010 && !StyleManager.IsMetro(effectiveStyle))
                    {
                        using (System.Drawing.Pen pen = new System.Drawing.Pen(pa.Colors.PanelBorder, 1))
                        {
		...
		protected internal override void OnItemAdded(BaseItem item)
		{
            ...
            item.ContainerState = eContainerState.NavigationPane;
            item.DesignMarkerOrientation = eDesignMarkerOrientation.Horizontal;
		}
        /// <summary>
        ...
        protected internal override void OnAfterItemRemoved(BaseItem item)
        {
            ...
            item.ContainerState = eContainerState.NotSet;
            item.DesignMarkerOrientation = eDesignMarkerOrientation.NotSet;
        }
