...
namespace DevComponents.DotNetBar.SuperGrid
{
    ...
    public class GridColumnHeader : GridElement
    {
        ...
        private static GridColumn _LastSelectedColumn;
        ...
        internal override void InternalMouseDown(MouseEventArgs e)
        {
            ...
            if (panel != null)
            {
                ...
                _LastSelectedColumn = _HitColumn;
            }
        ...
        {
            GridColumn lastCol = _LastSelectedColumn;

            if (lastCol != null)
            {
                int startColumnIndex = lastCol.ColumnIndex;

                int[] map = panel.Columns.DisplayIndexMap;

                if (extend == false)
                {
                    GridColumn column = panel.Columns[map[startColumnIndex]];

                    column.IsSelected = true;
                }
                else
                {
                    int anchorIndex = this._MouseDownHitColumn.ColumnIndex;
                    int endColumnIndex = _HitColumn.ColumnIndex;

                    panel.NormalizeIndices(true,
                        anchorIndex, ref startColumnIndex, ref endColumnIndex);
