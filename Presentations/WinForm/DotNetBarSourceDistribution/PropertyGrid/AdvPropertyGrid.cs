...
namespace DevComponents.DotNetBar
{
    ...
    public class AdvPropertyGrid : ContainerControl, INotifyPropertyChanged, IPropertyGridLocalizer, IPropertyElementStyleProvider, ISupportInitialize
    {
        ...
        internal bool InvokePropertyValueChanging(string propertyName, object newValue, PropertyDescriptor propertyDesc, string propertyPath)
        {
            PropertyValueChangingEventArgs e = new PropertyValueChangingEventArgs(propertyName, newValue, propertyDesc, propertyPath);
        ...
        /// </summary>
        [Description("Occurs after properties have been loaded into the property grid.")]
        public event EventHandler PropertiesLoaded;
        /// <summary>
        /// Raises PropertiesLoaded event.
        /// </summary>
        /// <param name="e">Provides event arguments.</param>
        protected virtual void OnPropertiesLoaded(EventArgs e)
        {
            EventHandler handler = PropertiesLoaded;
            if (handler != null)
                handler(this, e);
        }
        #endregion
        ...
        private void Reload(bool selectedObjectChanged)
        {
            ...
            OnPropertiesLoaded(EventArgs.Empty);
        }
        ...
        public SuperTooltip SuperTooltip
        {
            ...
            internal set { _SuperTooltip = value; }
        }
        ...
        {
            if ((char.IsLetterOrDigit(e.KeyChar) || char.IsPunctuation(e.KeyChar) || char.IsSeparator(e.KeyChar)) && _PropertyTree.SelectedNode is PropertyNode)
            {
        ...
        public void SetPropertyColumnWidth(int columnIndex, int width)
        {
            _PropertyTree.Columns[columnIndex].Width.Absolute = width;
            UpdateRelativeTreeColumnWidth();
        }

        private void UpdateRelativeTreeColumnWidth()
        {
        ...
        {
            PropertyNode node = FindPropertyNodeByText(_PropertyTree, propertyName, null, true) as PropertyNode;
            return node;
        }

        private static Node FindPropertyNodeByText(DevComponents.AdvTree.AdvTree advTree, string text, Node startFromNode, bool ignoreCase)
        {
            if (startFromNode == null)
                startFromNode = NodeOperations.GetFirstVisibleNode(advTree);
            else
                startFromNode = NodeOperations.GetAnyNextNode(startFromNode);
            ...
            while (node != null)
            {
                if (node is PropertyNode && ((PropertyNode)node).PropertyName.StartsWith(text, comparison))
                    return node;
                else if (node.Text.StartsWith(text, comparison))
                    return node;
                node = NodeOperations.GetAnyNextNode(node);
            }
        ...
        private ePropertySort _SubPropertiesDefaultSort = ePropertySort.Alphabetical;
        /// <summary>
        /// Gets or sets the sub property default sorting. Default value is Alphabetical.
        /// </summary>
        [DefaultValue(ePropertySort.Alphabetical), Category("Behavior"), Description("Indicates sub property default sorting.")]
        public ePropertySort SubPropertiesDefaultSort
        {
            get { return _SubPropertiesDefaultSort; }
            set
            {
                if (value != _SubPropertiesDefaultSort)
                {
                    ePropertySort oldValue = _SubPropertiesDefaultSort;
                    _SubPropertiesDefaultSort = value;
                    OnSubPropertiesDefaultSortChanged(oldValue, value);
                }
            }
        }
        /// <summary>
        ...
        /// </summary>
        ElementStyle IPropertyElementStyleProvider.DefaultEditCellStyle
        {
            get
            {
                if (_Appearance.DefaultEditCellStyle != null && _Appearance.DefaultEditCellStyle.Custom)
                    return _Appearance.DefaultEditCellStyle;
                return _DefaultEditCellStyle;
            }
        }
        /// <summary>
    ...
    public interface IPropertyElementStyleProvider
    {
        ...
        /// </summary>
        ElementStyle DefaultEditCellStyle { get;}
    }
    #endregion
    ...
    public class PropertyValueChangingEventArgs : EventArgs
    {
        ...
        /// <param name="propertyName"></param>
        /// <param name="newValue"></param>
        public PropertyValueChangingEventArgs(string propertyName, object newValue, PropertyDescriptor propertyDesc, string propertyPath)
        {
            PropertyName = propertyName;
            NewValue = newValue;
