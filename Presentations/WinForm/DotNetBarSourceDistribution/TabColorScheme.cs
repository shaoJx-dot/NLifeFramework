...
namespace DevComponents.DotNetBar
{
    ...
    public class TabColorScheme
    {
        ...
        public void Refresh()
        {
            ...
            switch (m_Style)
            {
                case eTabStripStyle.Metro:
                    {
                        if (StyleManager.IsVisualStudio2012(StyleManager.Style))
                            GenerateMetroVS2012ColorScheme();
                        else
                            GenerateMetroColorScheme();
                        break;
                    }
        ...
        private void GenerateMetroVS2012ColorScheme()
        {
            Office2007TabColorTable ct = null;
            Office2007ColorTable globalTable = null;
            if (GlobalManager.Renderer is Office2007Renderer)
            {
                globalTable = ((Office2007Renderer)GlobalManager.Renderer).ColorTable;
                if (_TabContext == eTabContext.DockTab && globalTable.DockTabStrip != null)
                    ct = globalTable.DockTabStrip;
                else
                    ct = globalTable.TabControl;
            }
            if (ct == null)
            {
                GenerateOneNoteColorScheme();
            ...
            // Tab Control Section
            if (!m_TabBackgroundCustom)
            {
                if (ct.TabBackground.Start.IsEmpty)
                    m_TabBackground = globalTable.Form.BackColor;
                else
                    m_TabBackground = ct.TabBackground.Start;
            }
            if (!m_TabBackground2Custom) m_TabBackground2 = ct.TabBackground.End;
            m_TabBackgroundGradientAngle = ct.TabBackground.GradientAngle;

            if (!m_TabBorderCustom) m_TabBorder = ct.TabPanelBorder;
            ...
            {
                if (ct.TabBackground.Start.IsEmpty)
                    m_TabItemText = globalTable.Form.TextColor;
                else
                    m_TabItemText = ct.Default.Text;
            }
            ...
            if (!m_TabItemHotBackgroundCustom && ct.MouseOver.TopBackground != null) m_TabItemHotBackground = ct.MouseOver.TopBackground.Start;
            ...
            // Tab Item Selected Section
            if (!m_TabItemSelectedBorderCustom)
            {
                if (ct.Selected.OuterBorder.IsEmpty)
                    m_TabItemSelectedBorder = m_TabItemText;
                else
                    m_TabItemSelectedBorder = ct.Selected.OuterBorder;
            }
            ...
            if (!m_TabItemSelectedBackgroundCustom && ct.Selected.TopBackground != null) m_TabItemSelectedBackground = ct.Selected.TopBackground.Start;
            ...
            // Tab Panel Section
            if (!m_TabPanelBackgroundCustom) m_TabPanelBackground = m_TabBackground;
            if (!m_TabPanelBackground2Custom) m_TabPanelBackground2 = m_TabBackground2;
            //m_TabPanelBackgroundGradientAngle=90;
            if (!m_TabPanelBorderCustom) m_TabPanelBorder = m_TabBorder;
        }

        private void GenerateMetroColorScheme()
        {
            Office2007TabColorTable ct = null;
            Office2007ColorTable globalTable = null;
            if (GlobalManager.Renderer is Office2007Renderer)
            ...
            if (!m_TabItemSelectedBorderLightCustom) m_TabItemSelectedBorderLight = Color.Empty;
            if (!m_TabItemSelectedBorderDarkCustom) m_TabItemSelectedBorderDark = Color.Empty;
            if (!m_TabItemSelectedBackgroundCustom) m_TabItemSelectedBackground = m_TabBackground;
            if (!m_TabItemSelectedBackground2Custom) m_TabItemSelectedBackground2 = Color.Empty;
            //m_TabItemSelectedBackgroundGradientAngle=90;
            if (!m_TabItemSelectedTextCustom) m_TabItemSelectedText = ct.Selected.Text;
        ...
        private eTabContext _TabContext = eTabContext.Default;
        internal eTabContext TabContext
        {
            get { return _TabContext; }
            set { _TabContext = value; }
        }
        #endregion
