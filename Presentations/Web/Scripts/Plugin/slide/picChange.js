...
* picChange lib
* 自动生成图片切换导航，以及图片轮转特效
* @name picChange-0.3.3.js
* @author  http://www.ilovespringna.com/
* @version 0.3.3
* @date Jun 5th, 2010
* @copyright (c) 2010
* @example or demo Visit
*/
...
function $pic(id) {
    ...
    return {
        ...
        picChange: function (param) {
            ...
            var showDesc = param.showDesc || false;
            ...
            if (picList[0].firstChild.tagName == "IMG")
                picList[0].firstChild.style.display = "block";
            else
                picList[0].firstChild.firstChild.style.display = "block";
            var picNum = picList.length; //获得图片数量

            //切换函数封装
            var changePicFunc = function (currentImg) {
                //若li元素下不为img元素，则在往下定位一层
                if (currentImg.tagName != "IMG")
                    currentImg = currentImg.firstChild;
            ...
            if (showDesc == true && document.getElementById("picDesc") == null) {
                ...
                if (firstPic.tagName != "IMG")
                    firstPic = firstPic.firstChild;
