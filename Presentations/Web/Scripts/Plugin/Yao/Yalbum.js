...
YAO.YAlbum.prototype.CARSOUEL_STEP_WIDTH = 700;
...
YAO.YAlbum.prototype.IMG_BTN_PREVIOUS = 'url(/Content/Images/Picture/arrow_left.cur),pointer';
YAO.YAlbum.prototype.IMG_BTN_NEXT = 'url(/Content/Images/Picture/arrow_right.cur),pointer';
YAO.YAlbum.prototype.SHARDOW_ID = 'carousel_photo_shardow';
YAO.YAlbum.prototype.LOAD_IMG_PATH = '/Content/Images/Picture/loading.gif';
...
YAO.YAlbum.prototype.btnsEnabled = function(){
	if (this.lastIndex !== 0 && YAO.hasClass(this.btnPrevious, this.BTN_DISABLED_CLASS)) {
		...
		if (YAO.ua.ie) {
		    this.btnPrevious.style.cursor = this.IMG_BTN_PREVIOUS;
		}
		...
		if (this.lastIndex === 0) {
			...
			if (YAO.ua.ie) {
			    this.btnPrevious.style.cursor = 'default';
			}
	...
	if (this.lastIndex !== (this.length - 1) && YAO.hasClass(this.btnNext, this.BTN_DISABLED_CLASS)) {
		...
		if (YAO.ua.ie) {
		    this.btnNext.style.cursor = this.IMG_BTN_NEXT;
		}
		...
		if (this.lastIndex === (this.length - 1)) {
			...
			if (YAO.ua.ie) {
			    this.btnNext.style.cursor = 'default';
			}
...
//    alert(width + "," + height);
    ...
    if (YAO.ua.ie) {
        ...
        YAO.setStyles(this.btnPrevious, {
            height: height + 'px',
            cursor: oSelf.IMG_BTN_PREVIOUS
        });
        YAO.setStyles(this.btnNext, {
            height: height + 'px',
            cursor: oSelf.IMG_BTN_NEXT
        });
    }
    if (this.lastIndex === 0) {
        YAO.addClass(this.btnPrevious, this.BTN_DISABLED_CLASS);
...
YAO.YLightBox = {
	...
	YPhoto: null,
	YPhotoDefaultWidth: 0,
	YPhotoDefaultHeight: 0,
	YSmall: null,
	YScroller: null,
	YTips: null,
	YIndex: null,
	...
	init: function(itemClass, itemTag, rootId){
		...
		for (i = 0; i < this.length; ++i) {
			YAO.on(this.photos[i], 'click', function(index){
				return function(event){
					var evt = event || window.event;
					this.YLastIndex = index;
					this.YPhotoDefaultWidth = this.photos[this.YLastIndex].width;
					this.YPhotoDefaultHeight = this.photos[this.YLastIndex].height;
					this.show();
					this.zoom();
					YAO.stopEvent(evt);
				}
	...
	close: function(event){
		var evt = event || window.event;
		if (this.YOverlay && this.YBox && this.isDisplay) {
	...
	show: function(){
		var D = document, i, j, oDf = D.createDocumentFragment(), docWidth = YAO.getDocumentWidth(), docHeight = YAO.getDocumentHeight(), oSelf = this;
		oImage = this.photos[this.YLastIndex];
		if (!this.YBox) {
			...
			this.YBoxAdjustBar = YAO.getEl('yl-box-adjust');
			this.YBoxNewWinBar = YAO.getEl('yl-box-newindow');
			this.YBoxBd = YAO.getEl('yl-box-bd');
			this.YPhoto = YAO.getEl('yl-photo');
			this.YSmall = YAO.getEl('carousel');
			this.YCarouselCnt = YAO.getEl('carousel_container');
			this.YScroller = YAO.getEl('samples_list');
			this.YBtnLastGroup = YAO.getEl('carousel_btn_lastgroup');
			this.YBtnNextGroup = YAO.getEl('carousel_btn_nextgroup');
			this.YTips = YAO.getEl('yl-tips');
			this.YIndex = YAO.getEl('yl-curIndex');
			
			for (i = 0; i < this.length; ++i) {
				this.YScroller.appendChild(YAO.Builder.Node('li', {
			...
			this.YAlbum = new YAO.YAlbum({
				btnPrevious: oSelf.YBtnLastGroup,
				Container: oSelf.YCarouselCnt,
				Scroller: oSelf.YScroller,
				btnNext: oSelf.YBtnNextGroup,
				stepWidth: 100,
				photoContainer: oSelf.YBoxBd,
				photo: oSelf.YPhoto,
				photoIntro: oSelf.YBoxTitle,
				photoMaxWidth: oSelf.YBoxDefaultWidth
			});
			for (j = 0; j < this.length; ++j) {
				YAO.on(this.YAlbum.oSamples[j], 'click', function(index){
					return function(){
						this.YLastIndex = index;
						this.chgPhoto();
					}
				}(j), this.YAlbum.oSamples[j], this);
			}
			
			YAO.on(this.YAlbum.btnPrevious, 'click', this.previous, this.YAlbum.btnPrevious, this);
			YAO.on(this.YAlbum.btnNext, 'click', this.next, this.YAlbum.btnNext, this);
			
			YAO.on(this.YBoxCloseBar, 'click', this.close, this.YBoxCloseBar, this);
			YAO.on(this.YBoxAdjustBar, 'click', this.adjustSize, this.YBoxAdjustBar, this);
			YAO.on(window, 'scroll', this.fixPosition, window, this);
			YAO.on(window, 'resize', this.fixPosition, window, this);
			YAO.on(this.YTips, 'click', this.showCarousel, this.YTips, this);
		}
		else {
			if (this.YOverlay && this.YBox && YAO.getStyle(this.YOverlay, 'display') === 'none' && YAO.getStyle(this.YBox, 'display') === 'none') {
				this.YBoxTitle.innerHTML = oImage.alt;
				this.YBoxNewWinBar.href = oImage.src;
				YAO.Builder.Attributes(this.YPhoto, {
					src: oImage.src,
					alt: oImage.alt
				});
				this.YIndex.innerHTML = (this.YLastIndex + 1);
				YAO.setStyle(this.YOverlay, 'display', 'block');
				YAO.setStyle(this.YBox, 'display', 'block');
			}
		}
		YAO.removeClass(this.YAlbum.lastSample,'current');
		YAO.addClass(this.YAlbum.oSamples[this.YLastIndex],'current');
		this.YAlbum.lastIndex = this.YLastIndex;
		this.YAlbum.lastSample = this.YAlbum.oSamples[this.YLastIndex];		
		this.isDisplay = true;
	},
	zoom: function(){
		var percent = this.YPhotoDefaultWidth / this.YPhotoDefaultHeight, height = this.YBoxDefaultWidth / percent;
		this.YBoxDefaultHeight = height + this.YBoxHdHeight;
		this.YBoxBDefaultHeight = height;
		if (this.YPhotoDefaultWidth > this.YBoxDefaultWidth) {
			this.zoomIn();
			this.YBoxAdjustBar.title = '显示实际尺寸';
		}
		else {
			this.zoomOut();
			this.YBoxAdjustBar.title = '显示缩略图';
		}
		this.fixPosition();
	},
	zoomIn: function(){
		var oSelf = this;
		YAO.setStyles(this.YPhoto, {
		...
		this.YBoxAdjustBar.title = '显示实际尺寸';
		if (YAO.ua.ie) {
			YAO.setStyle(this.YAlbum.btnPrevious, 'height', (this.YBoxDefaultHeight + 'px'));
			YAO.setStyle(this.YAlbum.btnNext, 'height', (this.YBoxDefaultHeight + 'px'));
		}
	},
	zoomOut: function(){
		var oSelf = this;
		YAO.setStyles(this.YPhoto, {
			width: oSelf.YPhotoDefaultWidth + 'px',
			height: oSelf.YPhotoDefaultHeight + 'px'
		});
		YAO.setStyles(this.YBoxBd, {
			width: oSelf.YPhotoDefaultWidth + 'px',
			height: oSelf.YPhotoDefaultHeight + 'px'
		...
		YAO.setStyles(this.YBox, {
			width: oSelf.YPhotoDefaultWidth + 'px',
			height: (oSelf.YPhotoDefaultHeight + oSelf.YBoxHdHeight) + 'px'
		});
		this.YBoxAdjustBar.title = '显示缩略图';
		if (YAO.ua.ie) {
			YAO.setStyle(this.YAlbum.btnPrevious, 'height', (this.YPhotoDefaultHeight + 'px'));
			YAO.setStyle(this.YAlbum.btnNext, 'height', (this.YPhotoDefaultHeight + 'px'));
		}
	},
	adjustSize: function(event){
		var evt = event || window.event;
		if (this.YPhotoDefaultWidth > this.YBoxDefaultWidth) {
			if (this.YPhoto.width === this.YBoxDefaultWidth) {
				this.zoomOut();
			}
			else {
				this.zoomIn();
	...
	showCarousel: function(){
		var top = parseInt(YAO.getStyle(this.YSmall, 'top'), 10);
		if (top === -120) {
			YAO.moveElement(this.YSmall, 0, 0, 50);
			YAO.setStyle(this.YBtnLastGroup, 'display', 'block');
			YAO.setStyle(this.YBtnNextGroup, 'display', 'block');
		}
		else {
			YAO.moveElement(this.YSmall, 0, -120, 50);
			YAO.setStyle(this.YBtnLastGroup, 'display', 'none');
			YAO.setStyle(this.YBtnNextGroup, 'display', 'none');
		}
	},
	previous: function(){
		this.YLastIndex -= 1;
		if (this.YLastIndex < 0) {
			this.YLastIndex = 0;
		}
		this.chgPhoto();
	},
	next: function(){
		this.YLastIndex += 1;
		if (this.YLastIndex > (this.length - 1)) {
			this.YLastIndex = this.length - 1;
		}
		this.chgPhoto();
	},
	chgPhoto: function(){
		...
		this.YPhotoDefaultHeight = this.photos[this.YLastIndex].height;
		this.YIndex.innerHTML = (this.YLastIndex + 1);
		this.YPhoto.alt = this.photos[this.YLastIndex].alt;
		this.YBoxNewWinBar.href = this.photos[this.YLastIndex].src;
		this.zoom();
	}
};